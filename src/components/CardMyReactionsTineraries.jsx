...
export default function CardMyReactionsTineraries({ reaction }) {
  ...
  const deleteReactionClick = () => {
    ...
    Alert.alert(`Your reaction in ${reaction.itineraryId.name} has deleted succesfuly`);
  };
    ...
    <View key={reaction._id} style={styles.cardReaction}>
      <View style={styles.infoReaction}>
        <Image
          style={styles.image}
          source={{
            uri: `${reaction?.itineraryId.photo[0]}`,
          }}
        />
      </View>
      <View style={styles.infoReaction}>
